...
def _sample_video_frames(
    ...
    """Internal helper for sample video frames."""
...
def _video_part_to_image_urls(part: dict[str, Any], resize: int | None = None) -> list[str]:
    """Internal helper for video part to image urls."""
...
def _pil_to_jpeg_data_url(image: Image.Image, max_side: int | None) -> str:
    """Convert to RGB, optionally bound the long edge by max_side, and return a JPEG data URL."""
...
def _get_image_size_from_data_url(data_url: str) -> tuple[int, int]:
    """Internal helper for get image size from data url."""
...
def build_gpt_response_create_kwargs(
    ...
    # Add reasoning only for GPT-5.1 models.
