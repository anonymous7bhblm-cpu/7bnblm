...
class BaseVideoICL:
    """Base class for Video ICL.
    Classification / Open-ended QA should inherit this class
    and override dataset-dependent methods.
    """
    ...
    def __init__(self, args):
        ...
        if self.model_name == "gemini":
            from google.genai import (
                errors,  # Import here because each model uses a different environment.  # noqa: F401
            )
    ...
    def _get_git_hash() -> str:
        """Internal helper for get git hash."""
        try:
    ...
    def prepare_inputs(self, messages: list[dict[str, Any]]):
        ...
        if self.model_name == "qwen3":
            ...
            # Return 0 because exact GPT token counting is difficult beforehand.
            # The actual input tokens are added to self.all_context_num in
            # process_inference from responses.create() usage.input_tokens.
            return inputs, 0
        else:
            ...
            try:
                for attempt in range(self.max_retries):
                    try:
                        ...
                        # File storage limit exhaustion is not resolved by retrying.
                        storage_exhausted = (
                            code == 429
                            and "file_storage_bytes" in err_text
                        )
                        # Daily quota exhaustion is not resolved on the same day, so fail immediately.
