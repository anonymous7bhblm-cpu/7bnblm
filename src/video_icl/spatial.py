...
class SpatialVideoICL(BaseVideoICL):
    ...
    def _get_selected_frame_ids(self, video_path, max_frames_num):
        """Internal helper for get selected frame ids."""
    ...
    def prepare_messages(self, demonstrations, query_id):
        ...
        query_video_path = os.path.join(self.video_data_dir, "clips4spatial", video_name)
        if self.text_only:
            selected_source_frame_ids = [
                int(item.get("frame_id", item.get("timestamp")))
    ...
    def inference(self):
        """Run the inference pipeline for spatial grounding."""
