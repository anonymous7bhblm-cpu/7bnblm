"""Send raw spatio-temporal grounding prediction JSON to Vidi evaluation.

This script performs the following steps:

1. Convert model output JSON to `tubes.csv`.
2. Run Vidi evaluation with the corresponding GT `tubes.csv` / `tubes_fps1.csv`.
3. Save `summary.csv`.

For 2-shot runs, the evaluation target is the corresponding domain under
`data/{domain}/vidi_eval/` rather than the individual dataset.
"""
import argparse
import ast
import csv
...
def parse_datetime_timestamp(value) -> datetime | None:
    """Convert supported datetime strings to datetime, otherwise return None."""
...
def parse_prediction_text(text: str) -> list[dict]:
    """Normalize a model output string into a bbox sequence.

    Expected input formats:
    - Strings with JSON code fences.
    - Python dict/list-like strings.
    - Lists of `{"time": ..., "bbox_2d": [...]}`.
    - Dicts of `{"boxes": [{"timestamp": ..., "bbox": [...]}]}`.

    Returns a list of dicts whose items contain `time` and `bbox_2d`.
    """
    text = text.strip()
...
def normalize_qwen_bbox(bbox: list[float]) -> list[float]:
    """Normalize Qwen-style 0-1000 bboxes to the 0-1 range."""
...
def normalize_gemini_bbox(bbox: list[float]) -> list[float]:
    """Normalize Gemini-style yxyx 0-1000 bboxes to xyxy in the 0-1 range."""
...
def sample_video_frame_indices(video_path: str, fps: float, max_frames: int) -> tuple[list[int], float]:
    """Reproduce the video sampling rule used for GPT."""
...
def infer_dataset_from_prediction_path(source_json: Path) -> str:
    """Infer the dataset name from the suffix of a bundle input JSON filename."""
...
def resolve_query_video_path(source_json: Path, query_video_path: str) -> str:
    """Resolve the real video path from query_video_path in a prediction sample."""
    ...
    return str(Path("data") / dataset_dir / "videos" / "0" / video_name)
...
def resolve_query_video_path_for_task(
    ...
    """Resolve the real video path for evaluation according to the task."""
    if osp.exists(query_video_path):
        return query_video_path
...
def build_prediction_text(sample: dict) -> str:
    """Extract the prediction text string from one sample.

    Check `prediction`, `pred`, and `raw_pred` in that order, and use the first
...
def _write_parse_failures_csv(target_csv: Path, failed_rows: list[list[str]]) -> None:
    """Write parse-failed samples to a CSV for manual inspection."""
...
def convert_prediction_json_to_vidi_tubes_default(
    ...
    """Convert Gemini/Qwen-style seconds plus 0-1000 bbox predictions to a Vidi prediction CSV."""
...
def convert_prediction_json_to_vidi_tubes_gpt(
    ...
    """Convert GPT-style frame_id plus 0-1 bbox predictions to timestamp(ms) plus 0-1 bbox rows."""
    data = json.loads(source_json.read_text())
...
def convert_prediction_json_to_vidi_tubes_llava_st(
    ...
    """Convert LLaVA-ST 100-bin spatio-temporal output to timestamp(ms) plus 0-1 bbox rows."""
    data = json.loads(source_json.read_text())
...
def run_vidi_evaluation(
    ...
    """Run Vidi evaluation with GT and prediction CSV files, then save summary.csv.

    If both `tubes.csv` and `tubes_fps1.csv` exist on the GT side, evaluate both
    and merge them into the same summary with a `variant` column.
    """
...
def parse_args() -> argparse.Namespace:
    """Define and return CLI arguments."""
...
def resolve_eval_target_name(dataset: str, source_json: Path) -> str:
    ...
    Standard runs such as 0-shot use the dataset name directly. For 2-shot runs,
    the source JSON stem ends with `_2shot`, so map the dataset name to the
