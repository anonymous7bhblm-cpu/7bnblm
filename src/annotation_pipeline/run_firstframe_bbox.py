"""Run object detection on annotations and first frames, then output bbox JSON."""
import argparse
import json
...
def parse_annotation_line(line):
    """Parse one annotation line into a video ID, timestamps, and query."""
...
class GroundingDinoDetector:
    """GroundingDinoDetector helper class."""
    ...
    def detect(self, image, query, box_threshold, text_threshold):
        """Return Grounding DINO detections from an image and query."""
        prompt = query.strip()
...
class OwlV2Detector:
    ...
    def detect(self, image, query, box_threshold, _text_threshold):
        """Return OwlV2 detections from an image and query."""
        prompt = query.strip()
        if not prompt.endswith("."):
            prompt = f"{prompt}."
...
def build_detector(args):
    """Build a detector for the specified backend."""
...
def main():
    """Run object detection on each first frame and save bbox JSON."""
    args = arg_parse()
